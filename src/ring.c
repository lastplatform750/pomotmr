...
void* ring_thread_func(void* arg) {
  ...
  while (!atomic_load(r->stop_requested)) {
    play_sound(alarm_fptr, r->rate, r->channels, r->stop_requested);
  }
...
int play_sound(int fptr, unsigned int rate, int channels,
...
cleanup:
  if (pcm_handle) {
    // If we didn't call snd_pcm_drop already and stop wasn't requested, do a
    // normal drain.
    if (!atomic_load(stop_requested))
